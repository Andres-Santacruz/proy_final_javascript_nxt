...
function imprimirPantalla(elemnt) {
  ...
  if (conte=='') {
      op2=2;
      valor2=valor;
      console.log('si  op: '+op2+'valor 2:  '+valor2);
  }else {
    op2=1;
    valor1=conte;
  ...
  switch (ver) {
    ...
    case "mas":
      opertecla=1;
      pan.innerHTML='';
      console.log('el conte que queda: '+conte+'  op:  '+op2);
      valor=conte;
    break;
    case "menos":
      opertecla=2;
    ...
    case 'raiz':
      x=pan.innerHTML;
      if(parseInt(x)>0){
        pan.innerHTML=''+ Math.sqrt(x).toFixed(7);
      }else {
        pan.innerHTML='';
      }
    break;
...
fd=document.getElementsByClassName('teclado');
...
for(i=0;i<fd.length;i++){
  fd[i].onmousedown=asigDisminuir;
  fd[i].onmouseup=asigAumento;
}
